...
%SC
%Finally, ``more'' is intepreted as ``at least
...
We presented a system using  GF, Dynamic Semantics and Coq, essentially converting GF trees to Coq types using dynamic semantics. The system outperforms the state of the art in logical approaches when tested on the FraCaS test suite and is the only system to date that has performed a run on the ellipsis/anaphora section of the FraCaS. The system's precision  can form the start of a very precise NL reasoner for controlled domains. For the future, we plan to  extend the system to cover the remaining section of the FraCaS (tense/aspect), and also develop a more applied version to perform reasoning on controlled NL domains. 

\bibliography{PaperTools/bibtex/jp.bib,phorae.bib,IWCSbib.bib,NLI.bib}
