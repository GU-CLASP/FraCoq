...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4.2cm,
                    semithick,scale=0.63, every node/.style={transform shape},inputnode/.style={draw=black,text=black,shape=rectangle,rounded corners=1mm}]
  \tikzstyle{every state}=[draw=black,text=black,shape=rectangle]

 \node[inputnode]         (txt)                {Textual Problem};
 ...
 \node[inputnode]         (proof) [above of=Coq,node distance=2cm]  {Proof};
 \path  (proof) edge                (Coq);
 \node               (peter) [above of=GF,node distance=3cm]  {[Ljunglöf 2011]};
 \path  (peter) edge                (GF);
 \node               (cooper) [above of=txt,node distance=2cm]  {[Cooper \textit{et al.} 1996]};
 \path  (cooper) edge                (txt);
