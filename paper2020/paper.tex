...
temporal section.\jp{check todo in footnote}\footnote{One can consider that \citet{maccartney:2007} have made a run against the whole test suite. However, they do not deal with multi-premise cases. Consequently only 36/75 cases in the temporal section are treated{TODO: jp: treated or attempted? I suppose attempted (otherwise .61 accuracy makes no sense)}. The general accuracy of the system is .59, and .61 for the temporal section. Our system, as shown \cref{tab:results}, presents considerable improvements in coverage and accuracy over that of \citeauthor{maccartney:2007}.}
%
...
Total        & 337        & .80        & .89         & .83    & .69  & .50  & .85  \\
  ...
  \label{tab:results}
\end{table}
\paragraph{interaction with group readings}
...
imperfectly with group readings.

We stress that group readings is already flawed in the system which we base our analysis on.
The issue is related to the difference
between narrow  and wide scope.\footnote{The literature on wide and narrow scope readings of quantifiers and quantifiers and negation is vast. We direct the reader to \cite{steedman:2012} for a comprehensive and thorough discussion of the relevant issues.} (In fact the FraCaS test suite does not appear to
...
Outside of inference, formal interpretation of tense is found in
natural-language interface to databases. Of note is the work of
...
interaction logical aspects of inference, such as coreference via
