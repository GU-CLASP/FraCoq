...
semantics of tense and aspect has been also considered in the study of
...
\citet{bernardy_type-theoretical_2017,bernardy_wide-coverage_2019}, which offers a complete whole run of the
...
account of tense and aspect to deal with the notorious section 7 of
...
also provide some brief remarks on temporal semantics. In \cref{sec:our-semantics},
we discuss the main aspects of the compositional semantics of our, using various examples
from the FraCaS suite in exemplifying its effectiveness. In \cref{sec:results},
...
section. In \cref{sec:conclusions}, we conclude and discuss avenues for future
work.
...
\cite{unger_dynamic_2011,charlow_monadic_2015,charlow_modular_2017}.
Our particular implementation is not perfect \ifanon (citations
anonymized) \else \citet{bernardy_wide-coverage_2019} \fi but non-temporal
...
ones. We explain our precise meaning forthese terms in the body of the
paper. Nevertheless, we refer the reader to \citet{steedman:2000} for
an extensive review of temporal semantics.
...
single one.  As such, we think it is a good classification, given that
they generally afford the correct inferences for the testsuite. (In
this paper, we always assume that the FraCaS testsuite is correct.)
...
\newcommand\nospan{\ensuremath{-}}
\newcommand\anyspan{\ensuremath{\mathbf{?}}}\todo{better notation} The timespan in the context is
...
In our semantics we deal only with present and past
tenses (simple and continuous). Indeed we find that FraCas
...
Even though we will discuss a refinement to handle the past continuous
...
(The difference between actitivty and accomplishments on the one and
...
not nil. However, this appears to play little role, if any, in our
analysis, and given the its task-driven nature as well.)
...
problem (279) is worked out fully.

Input from \citet{Ljunglof:2012}:


\begin{verbatim}
s_279_1_p :: Phr
s_279_1_p=((sentence) ((useCl) ((past)) ((pPos)) ((predVP) ((usePN) ((lexemePN "smith_PN"))) ((advVP) ((complSlash) ((slashV2a) ((lexemeV2 "write_V2"))) ((detCN) ((detQuant) ((indefArt)) ((numSg))) ((useN) ((lexemeN "novel_N"))))) ((lexemeAdv "in_1991_Adv"))))))
s_279_3_h :: Phr
s_279_3_h=((sentence) ((useCl) ((past)) ((pPos)) ((predVP) ((usePN) ((lexemePN "smith_PN"))) ((advVP) ((complSlash) ((slashV2a) ((lexemeV2 "write_V2"))) ((usePron) ((it_Pron)))) ((lexemeAdv "in_1992_Adv"))))))
\end{verbatim}
Interpretation by our dynamic and temporal semantics:

\begin{verbatim}
Definition Problem279aFalse:= (FORALL (fun a=>novel_N a) (fun a=>(exists (b: Time), ((exists (c: Time), (IS_INTERVAL Date_19910101 b /\ IS_INTERVAL c Date_19911231 /\ IS_INTERVAL b c /\ appTime b c (write_V2 a) (PN2object smith_PN))))) -> Not (exists (f: Time), ((exists (g: Time), (IS_INTERVAL Date_19920101 f /\ IS_INTERVAL g Date_19921231 /\ IS_INTERVAL f g /\ appTime f g (write_V2 a) (PN2object smith_PN))))))).
\end{verbatim}
Unfolding definitions by Coq:

\begin{verbatim}
  forall x : object,
  novel_N x ->
  (exists b c : Z,
     Date_19910101 <= b /\ c <= Date_19911231 /\ b <= c /\ write_V2 x SMITH b c) ->
  (exists f g : Z,
     Date_19920101 <= f /\ g <= Date_19921231 /\ f <= g /\ write_V2 x SMITH f g) ->
  False
\end{verbatim}
Proof:
...
between narrow scoping and wide scoping \cite{todo}\todo{Stergios?}. Unfortunately,
...
\bibliography{../PaperTools/bibtex/jp,paper2020,../paper2018/NLI.bib}
