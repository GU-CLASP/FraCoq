...
Stergios
The semantics of tense and aspect has been a long standing issue in the study of formal semantics since the early days of Montague Grammar and a number of different ideas have been put forth on the table to deal with them throughout the years \citet{dowty:2012,prior:2003,steedman:2000,higginbotham:2009,fernando:2015}. The semantics of tense and aspect has been also central in the study of Natural Language Inference (NLI)\ The various datasets for NLI that have been proposed by the years contain examples that have some implicit or explicit reliance ton nferences related to tense and aspect. One of the early datasets used to test logical approaches, the FraCaS test suite ~\citetp{cooper:196} contins a whole section of the dataset only for temporal and aspectual inference (section 7). This part of the FraCaS test suite has been  difficult to tackle. In fact, all the computational systems ran on the FraCaS so far skip this section, and usuall. Additionally, they ora and ellipsis sns.The exception with respect to the ellipsis and anaphora sections is, with the exception of the system presented by \cibernardy_wide_2019}, which offeres a complete whole run of the FraCaS test uite, excluding the temporion as been atIn this paper, we take upge of providing a computationally viable account of tense and aspect to deal with the notorious section 8 of the FraCaS test suite. ThisOur account is not meant to be a theoretically full account of tense and aspect, but rather an acnt that is driven by the need to cover the test suite in a way that is general enough to captutest suite examples. The account is evaluated on the entailment properties of various temporal and aspectual examples, as given by the test suite. As such, we are not getting into the discussion of how tense and aspect might affect grammaticality or infelicitousness of various sentences. We take the sentences of the FraCaS suite to be syntactically correct and the entailment patterns they are annotated with valid.                    paper is structured as follows: in chapter 2\cref{sec:background}, we give a brief background of the system wsummary of the frameworks whose vare going to be using, saying a few words about its main individual components: the Grammatical Framework that is used asto construct the syntactic parser,; the Coq proof assistant which doechecks all the reasoning; and some brief remarks on temporal semantics \textbf{(JP: Anything to say on Dynamic Semantics?)}. In section 3, we discuss the main aspects of the compositional semantics of our system \textbf{This could go in the background actually}, and in section 4, we give our main treatment of time, using various examples from the FraCaS suite in exemplifying its effectiveness. In section 5, we havtake a brief look at how our system performs with respect to the temporal section of the FraCaS, but we also ran the system across the whole of the FraCaSsuite, this being the first time that a complete run through the whole suite has been made,
. In section 6, we conclude and discuss avenues for future work. 
