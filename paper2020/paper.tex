...
\todo{integrate to background}

Our interpretation is cast within the framework of a largely standard
montagovian semantics \cite{todo}. In brief, sentences are interpreted as
...
monadic-based dynamic semantics \cite{TODO}.  Our particular
...
the montagovian system over which we base our implementaiton does not
support this phenomena. (Indeed the test suite does not appear to
handle this case). \todo{You mean the system? It seems unclear what you are talking and in what way a test suite is not handling an example. It does not exist or?} We propose that the situation becomes
