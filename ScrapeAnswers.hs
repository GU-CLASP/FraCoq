...
-- allComments :: String -> IO (Maybe [String])
allComments :: URL -> IO (Maybe [(Int, Answer)])
allComments url = scrapeURLWithConfig def url posts
   where
     posts = chroots ("problem" @: []) $
               ((,) <$> (read <$> attr "id" anySelector) <*> (parseAnswer <$> text ("a" @: [])))

trimHalf :: [Char] -> [Char]
trimHalf = dropWhile isSpace . reverse

trim :: [Char] -> [Char]
trim = trimHalf . trimHalf

parseAnswer :: [Char] -> Answer
parseAnswer (trim -> a) = case a of
  "Yes" -> Yes
  "No" -> No
  "Don't know" -> Unknown
  _ -> Unclear a
...
main :: IO ()
main = do
  let url = "https://nlp.stanford.edu/~wcmac/downloads/fracas.xml"
  Just cs <- allComments url
  putStrLn (ppShow cs)
